...
#
# 4a function to evaluate abstraction rates
MM$qeval <- function(params,MM_qs0,MM_qf0,MM_qs2,MM_qf2) {
  qs0 <- MM_qs0(params) # get initial q_hat
  qf0 <- MM_qf0(params)
  qs1 <- qs0
  qf1 <- qf0
  ...
  for (i in 1:500) {
    if (qs1 != qs2 | qf1 != qf2) { # if constrained q_hat != initial q_hat
      ...
      qs1 <- qs2 # store old
      qf1 <- qf2
      qs2 <- apply_constraints(MM_qs2(params,qf1),c(0,params$Qs)) # update qshat with constrained qfhat
      qf2 <- apply_constraints(MM_qf2(params,qs1),c(0,params$Qf)) # update qfhat with constrained qshat
    } else {
...
MM$qfdouble0 <- function(params) { # in call: include params %>% mutate(qfhat=qfhat,qshat=qshat)
  with(params,
...
MM$qsdouble2 <- function(params,qf1) {
  with(params,
       (p0s-B*(d0s+Dsf*(qf1-gs*qf1+gs*qfhat)-rmT*rsT))/(2*B*Dss)
  )}
MM$qfdouble2 <- function(params,qs1) {
  with(params,
...
# 4c Mathematica functions for z constraints
# z constraints
MM$zMinSwiss <- function(params,q_vals) {
...
MM$zMaxFrench <- function(params,q_vals) {
  with(q_vals,
       with(params,
...
MM$zRange <- function(params,q_vals) {
  zRange <- MM$zMaxFrench(params,q_vals) - MM$zMinSwiss(params,q_vals)
  return(zRange)
}
