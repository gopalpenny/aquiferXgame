...
df_input_depleted <- data.frame(B=0.91, dBf=75.4, dBs=75.4, h0f=45.3, h0s=42.7, p0s=100, PHIff=110, PHIfrN=97.5, PHIfrT=97.5, PHIfs=91.4, PHIsf=91.4, PHIsrN=121, PHIsrT=121, PHIss=110, year=1956, row=17, Qs=17, Qf=0, gs=0.6, gf=0.6, p0f=5e+05, crs=5e+05, rmN=0, rmT=0, es=0, ef=0, l=1)
...
df_qdoublt_results <- suppressWarnings(evaluate_treaty_cases(df_input_qdouble,'qd'))
test_that('evaluate_treaty_cases returns qx_double equal to qi_hat and qi_star in cases where qj is 0',{
  expect_equal(df_qdoublt_results %>% dplyr::mutate_if(is.numeric,function(x) round(x,4)),
               df_output_qdouble)
})
